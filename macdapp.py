...
<style>
    ...
    .criterion-met { color: green; font-weight: bold; }
    .criterion-not-met { color: red; font-weight: bold; }
    ...
    /* Ensure score column aligns numbers correctly */
     ...
     .dataframe td[align="center"], .dataframe th[align="center"] { text-align: center !important; }
</style>
...
def check_strategy_setup(conditions_indicators, entry_indicators):
    ...
    if conditions_long_met:
        ...
        if entry_indicators.get('Daily_Price_Structure_Long', False): entry_rules_met_count_l += 1; rules_met_long_details.append("D:Prc>EMAs") # Use combined check
    ...
    if conditions_short_met:
        ...
        if entry_indicators.get('Daily_Price_Structure_Short', False): entry_rules_met_count_s += 1; rules_met_short_details.append("D:Prc<EMAs") # Use combined check
...
def display_results_table(results_list):
    ...
    # Helper to format rule status
    def format_rule_status(met):
        return '<span class="criterion-met">✅</span>' if met else '<span class="criterion-not-met">❌</span>'
    ...
    for i, r in enumerate(results_list):
        ...
        dp_gt_Es = ei.get('Daily_Price_Structure_Long', False) # Combined price structure check
        # Weekly Short Cond
        ...
        dp_lt_Es = ei.get('Daily_Price_Structure_Short', False) # Combined price structure check


        df_data.append({
            "Name": r["name"],
            # "Ticker": r["ticker"], # Excluded
            "Setup": setup_html,
            "Score": r["Score"],
            # --- Criteria Columns ---
            # Weekly Conditions
    ...
    # Default sort: High positive scores first, then low negative scores
    df_display = df_display.sort_values(by="Score", ascending=False).reset_index(drop=True)

    # Define column order (prioritize key info, group criteria)
    column_order = [
        "Name", "Setup", "Score",
        # Weekly Criteria
        "W:R>50", "W:M Bull", "W:P>E21",
        "W:R<50", "W:M Bear", "W:P<E21",
        # Daily Criteria
        "D:R>50", "D:M Bull", "D:P>Es",
        "D:R<50", "D:M Bear", "D:P<Es",
        # Internal Index (keep for selection logic, but hide if possible, or ensure last)
        "_original_index"
    ]
    # Reorder columns - handle potential missing columns if criteria change later
    ...
    # Return the *original* df_display which *includes* the _original_index for selection
    return df_display
...
# --- Main App Flow ---
def main():
    # ... (main function layout remains the same) ...
    ...
    with st.expander("📖 Strategy Criteria Explained"):
        ...
        **LONG Setup Criteria:**
        1.  **Weekly Conditions (ALL must be met):** `W:R>50`, `W:M Bull`, `W:P>E21`
        2.  **Daily Entry Confirmation (at least {MIN_ENTRY_RULES_MET} must be met):** `D:R>50`, `D:M Bull`, `D:P>Es` (Price > EMA {EMA_SHORT} & {EMA_LONG})

        **SHORT Setup Criteria:**
        1.  **Weekly Conditions (ALL must be met):** `W:R<50`, `W:M Bear`, `W:P<E21`
        2.  **Daily Entry Confirmation (at least {MIN_ENTRY_RULES_MET} must be met):** `D:R<50`, `D:M Bear`, `D:P<Es` (Price < EMA {EMA_SHORT} & {EMA_LONG})
    ...
    with tab_dashboard:
        ...
        if not st.session_state.scan_results: st.info("Click 'Run Scan' in the sidebar to start.")
            ...
            if not displayable_results: st.success("Scan complete. No active Long/Short/Watch setups found.")
                ...
                displayed_df = display_results_table(displayable_results)
